...
uint8_t LED_update_halfcycle(uint8_t*, uint16_t*, uint8_t);
void LED_update_cycle(void);


uint16_t LED_pwm[8] = {130,41,60,131,10,21,60,21}; 
uint8_t LED_bit[8] = {1,2,4,8,16,32,64,128};
...
static const uint16_t LED_halfcycle_end = 200;
...
uint8_t ii;
for (ii = 0; ii < n; ii++){
printf("LED_index[%d] = %d\n", ii, LED_index[ii]);
}
for (ii = 0; ii < n; ii++){
printf("LED_pwm[%d] = %d\n", LED_index[ii], LED_pwm[LED_index[ii]]);
}
printf("\n");
...
for (ii = 0; ii < n; ii++){
printf("LED_start[%d] = %d\n", LED_index[ii], LED_start[LED_index[ii]]);
}
for (ii = 0; ii < n; ii++){
printf("LED_end[%d] = %d\n", LED_index[ii], LED_end[LED_index[ii]]);
}
printf("\n");
...
*/
	...
	// test WPM timing and P1OUT calculation
	LED_update_cycle();
	uint8_t i;
	for (i = 0; i < 10; i++) {
		printf("LED_out[%d] = %d\n", i, LED_out[i]);
